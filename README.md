...
In this section, we will explain the similarity between a feed forward neural network and RNN by building an unrolled version of Vanilla RNN (using a feedforward neural network). A Vanilla RNN has a simple hidden state matrix (memory) and is easy to understand/implement.  
...
You can check this [blog post](https://www.oreilly.com/ideas/sentiment-analysis-with-apache-mxnet) to learn about embedding layer) and its functionality.  The Dense layer 1, 2 and 3 learn a set of weights that can predict the 4th character given the first 3 characters.
...
```
6. You can download the MXNet notebook for this part of the tutorial [here](https://github.com/sookinoby/RRN-MxNet/blob/master/Test-rnn.ipynb), where we've created and run all this code, and play with it! Adjust the hyperparameters and experiment with different approaches to neural network architecture.
...
In the previous section, we prepared dataset for predicting 4th character given the previous 3 characters. In this section, we will prepare dataset for predicting nth character given arbitrary sequence length of n-1. This is very similar to preparing the dataset for unrolled RNN, except for the shape of the input. The dataset should be ordered in the shape (number of example X batch_size). Now, let us batch the sample dataset below:
![Alt text](images/batch3.png?raw=true "batch reshape") <br />
In the above image, the input sequence is converted to a batch size of 3 and sequence length of 4 . By transforming it this way, we lose the temporal relationship between 'O' and 'V', 'M' and 'X'. For example, ‘V’ follows ‘O’  in the input sequence but ‘V’ and ‘O’ belong to different batches; The only reason we batch the input sequence is to train our model faster. The following python function does the batching of input
```python
def rnn_batch(data, batch_size):
    ...
    return data
```
Below is the example with batch size 2 ![Alt text](images/batch4.png?raw=true "batch reshape") <br /> and sequence length of 6.  It is very easy to generate the arbitrary length input sequence from a given batch. For example for from a batch size of 2 , if we want a sequence generate sequence of length 3, it  is quite easy as shown in the picture below. Only after deciding on the sequence length, we can man the input sequence to output sequence (target), and the following python function does the work for us.
```python
#get the batch
def get_batch(source, i,seq):
    ...
    return data, target.reshape((-1,))
```
  It is very easy to generate the arbitrary length input sequence. During our training, we use an input sequence length of 100. This is a hyperparameter and may require fine tuning for the best output.
...
### Text generation
Once the model is trained with the input data, we can generate random text similar to the input data. The weights of the trained model (200 epochs) with sequence length of 100  is available [here](https://www.dropbox.com/s/7b1fw94s1em5po0/gluonlstm_2?dl=0). You can download the model weights and load it using [model.load_params](https://mxnet.incubator.apache.org/api/python/module/module.html?highlight=load#mxnet.module.BaseModule.load_params) function. Next, we will use this model to generate text similar to 'nietzsche'. To generate sequence of arbitrary length, we use the output generated by the model as the input for the next iteration. 
To generate text, we initialize the hidden state.
```python
 hidden = model.begin_state(func = mx.nd.zeros, batch_size = batch_size, ctx=context)
```
Remember, we don't have to reset the hidden state as we don’t backpropagate the loss to fine tune the weights.
...
```python
 new_string = new_string + indices_char[index[-1]]
        input_string = input_string[1:] + indices_char[index[-1]]
```
Below is the function snippets that generates ‘'nietzsche' like text of arbitrary length.
```python
...
Next, we will take a look at generative models*, specially Generative Adversarial Network  a powerful model that can generate new data from a given input dataset.
